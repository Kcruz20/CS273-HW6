	...
	*/
	std::vector<int> find_accounts_by_name(std::string name)
	{
		...
		for (int i = 0; i < accounts.size(); i++) {
			Customer *temp = accounts[i]->get_customer();
			if (temp->get_name() == name)
				user_accounts.push_back(temp->get_id());
		}
	...
	*/
	Account * add_account(Customer *cust, std::string account_type)
	{
		...
		if (account_type == "savings")
			...
			std::cout << "Invalid account type. Accounts can only be savings or checkings.\n";
			return NULL;
		}
		account_id++;
	...
	*/
		...
		std::string cust_type, std::string account_type)
	{
		...
		if (cust_type == "student")
			cust = new Student(name, address, age, telephone, cust->get_id());
		else if (cust_type == "adult")
			cust = new Adult(name, address, age, telephone, cust->get_id());
		else if (cust_type == "senior")
			cust = new Senior(name, address, age, telephone, cust->get_id());
	...
	*/
	void make_deposit(int acct_number, double amt)
	{
		...
		if (acct) {
			...
			int currbalance = acct->get_balance();
			currbalance += amt;
			acct->set_balance(currbalance);
		}
	}
